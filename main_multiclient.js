...
const workloadName = config.workload;
let workloadPath = config[workloadName].path; // config.workloadPath = path to the class implementing the Load interface
...
const endorsementPolicy = parseInt(config.endorsementPolicy.split("/")[0]); // policy can be "1/4" or "1s/4" etc. s means short policy (using noutof). parseInt will ignore following s
    ...
    for(let i = 0, peerId = 0, orgId = 0, chanId = 0, ordererId = 0; i < config.numProcesses; i++) {
        ...
        .on('message', (msg) => {
            ...
            case PROPOSAL_LATENCY:
                proposalLatencyFile.write(`${msg.st} ${msg.v} ${msg.tId} ${msg.c} ${msg.S}\n`);
                d = (new Date()).toString();
                broadQueueLenFile.write(`${d} ${msg.c} ${++broadcastQueueLen}\n`);
                commitQueueLenFile.write(`${d} ${msg.c} ${++commitQueueLen}\n`);
                return;
            case BROADCAST_LATENCY:
                broadcastLatencyFile.write(`${msg.st} ${msg.v} ${msg.tId} ${msg.c} ${msg.S}\n`);
                broadQueueLenFile.write(`${new Date()} ${msg.c} ${--broadcastQueueLen}\n`);
                return;
            case COMMIT_LATENCY:
                commitLatencyFile.write(`${msg.st} ${msg.v} ${msg.tId} ${msg.c} ${msg.S}\n`);
                commitQueueLenFile.write(`${new Date()} ${msg.c} ${--commitQueueLen}\n`);
                return;
            case TOTAL_LATENCY:
                totalLatencyFile.write(`${msg.st} ${msg.v} ${msg.tId} ${msg.c} ${msg.S}\n`);
                return;
            default: // fallthrough
            }
            stats.push([i, msg]);
